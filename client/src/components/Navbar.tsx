    ...
    <nav className="sticky top-0 z-40 w-full border-b border-amber-100/50 bg-gradient-to-r from-white via-amber-50/30 to-rose-50/30 shadow-sm backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          >
            <span className={`h-0.5 w-6 rounded-full bg-stone-700 transition-all ${mobileMenuOpen ? "translate-y-2 rotate-45" : ""}`} />
            <span className={`h-0.5 w-6 rounded-full bg-stone-700 transition-all ${mobileMenuOpen ? "opacity-0" : ""}`} />
            <span className={`h-0.5 w-6 rounded-full bg-stone-700 transition-all ${mobileMenuOpen ? "-translate-y-2 -rotate-45" : ""}`} />
          </button>
        </div>
